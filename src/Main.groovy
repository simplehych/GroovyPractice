...
def clearScreenShotCacheWhenFinishTask = false           //Optional，是否自动清除截图缓存
...
println "————————————————————————————————————"
println "adb device".execute().text
println "————————————————————————————————————"
println "开始执行自动阅读"
...
def task_screenShot(String rootPath) {
    ...
    println "adb shell screencap -p /sdcard/shanbey/${millis}".execute().text
    println "adb pull /sdcard/shanbey/${millis} $screenShotPath".execute().text
